...
- To extend the range, each board forwards messages it receives to its neighbors, creating a mesh network.

- To make sure messages don’t loop indefinitely, make very device acknowledge messages it has seen before and only forward new messages.

- Until all boards have sent an acknowledgment for a message, it is forwarded to all neighbors. Once all acknowledgments are received, the message is considered delivered and is not forwarded anymore.
